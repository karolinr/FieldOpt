...
public:
    ...
    QList<Well> wells() { return wells_; } //!< Get the struct containing settings for the well(s) in the model.
    QList<int> control_times() { return control_times_; } //!< Get the control times for the schedule
    void set_control_times( QList<int> control_times){control_times_ = control_times;}//!< Set the control times through GUI
private:
